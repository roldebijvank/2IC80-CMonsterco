...
pub fn decrypt_folder_parallel(folder_path: &Path, pk: &PublicKey, sk: &SecretKey) -> Result<()> {
    ...
    println!("decryption completed for {:?}", folder_path);
    Ok(())
}

fn decrypt_recursive_parallel(root: &Path, current: &Path, pk: &PublicKey, sk: &SecretKey) -> Result<()> {
    if current.is_file() {
